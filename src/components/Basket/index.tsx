...
export const Basket = ({ api }: { api: boolean }) => {
    ...
    const orderItems = React.useMemo((): Order[] | [] => {
        if (orders === null) return [];
        return ObjectUtil.toArray(orders as Record<string, Order>);
    }, [orders]);
    ...
    const handleSubmit = React.useCallback(
        ...
        [api, dispatch, enqueueSnackbar, orders]
    );

    const orderActive = orderTotal > 0;

    return <InnerBasket handleSubmit={handleSubmit} orderTotal={orderTotal} orderItems={orderItems} orderActive={orderActive} emptyBasket={emptyBasket} />;
};
...
export const InnerBasket = React.memo(({ handleSubmit, orderTotal, orderItems, orderActive, emptyBasket }: Props) => (
    <Box
        sx={{
            p: 1,
            m: 1,
...
InnerBasket.displayName = "InnerBasket";
